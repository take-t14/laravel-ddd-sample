...
- ドメインモデル→ViewModelを、ドメインモデルの配列化で実現しているが、大量データ取得時のパフォーマンスはMVCと比べると悪い
